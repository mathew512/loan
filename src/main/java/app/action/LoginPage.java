...
public class LoginPage extends HttpServlet {
    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        out.println("<div class='footer'><p><a href='#'>Forgot password?</a></p></div>");
        out.println("</form>");
        out.println("</div>");
        out.println("</body>");
        out.println("</html>");
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        if (username != null && password != null &&
            username.equalsIgnoreCase(usernameConfig) &&
            password.equalsIgnoreCase(passwordConfig)) {

            HttpSession session = request.getSession(true);
            session.setAttribute("UserActualName", username);
            session.setAttribute("SESSION_ID", session.getId());

            // ✅ Redirect into /Loan/home
            response.sendRedirect(request.getContextPath() + "/home");

        } else {
            request.getSession().invalidate();
            // ✅ Redirect back into /Loan/login
